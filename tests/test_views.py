...
def test_get_card_info_returns_correct_structure(
    ...
    for item in result:
        ...
        # Кэшбэк должен быть 1% от total_spent
...
# 3. Тест для функции get_top_five_transactions
def test_get_top_five_transactions_returns_top_five(mock_views_logger: Mock) -> None:
    """
    Тест функции get_top_five_transactions: проверяет возврат топ-5 транзакций.
    """
    # Создаем тестовый DataFrame с правильными столбцами
    test_df = pd.DataFrame(
        {
            "Сумма платежа": [500.00, 400.00, 300.00, 200.00, 100.00],
            "Сумма операции": [500.00, 400.00, 300.00, 200.00, 100.00],  # Добавляем этот столбец
    ...
    # Проверяем, что отсортировано по убыванию суммы платежа
    amounts = [item["amount"] for item in result]
    assert amounts == [500.00, 400.00, 300.00, 200.00, 100.00]  # В порядке убывания
...
# 4. Тест для функции get_currency_course
...
def test_get_currency_course_calls_api_correctly(
    ...
    """Тест функции get_currency_course: проверяет вызов API."""
...
# 5. Тест для функции get_stock_price
...
def test_get_stock_price_calls_api_correctly(
    ...
    """Тест функции get_stock_price: проверяет вызов Finnhub API."""
    # Настраиваем мок для finnhub.Client
...
# 6. Тест для функции get_main_web_json_answer
def test_get_main_web_json_answer_returns_valid_json(mock_views_logger: Mock) -> None:
    """Тест функции get_main_web_json_answer: проверяет валидность JSON ответа."""
    ...
    with (
        ...
        # Проверяем, что greeting был вызван с правильным часом
